...
export class ApiService {
  ...
  async getMostPopularProductsBySexAsync(sex: string, signal?: AbortSignal): Promise<BasicProductResponse[]> {
    const response = await fetch(`${this.baseUrl}/products/most-popular?sex=${encodeURIComponent(sex)}`, {
      method: "GET",
      headers: { ...this.requestHeaders },
      signal,
    });
